...
module.exports = class HelpCommand extends Command {
  ...
  static aliases = [undefined, 'h', '--help', '-h'];
  works = 'everywhere';
  ...
  _printHelp(commandOptions, rawArgs) {
    ...
    if (rawArgs.length === 0) {
      ...
      let multipleCommands = [GenerateCommand.name].concat(GenerateCommand.aliases);
  ...
  _resolveCommand(commandName) {
    let Command = this.commands[stringUtils.classify(commandName)] || this._lookupCommand(this.commands, commandName);
