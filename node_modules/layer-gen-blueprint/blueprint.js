...
class Blueprint {
  ...
  */
  ...
  constructor(blueprintPath, blueprintOptions) {
    this.pkgPath = findUp.sync('package.json');

    if (this.pkgPath) {
      this.pkg = require(findUp.sync('package.json'));
    }
  ...
  */
  ...
  // TODO remove this actions object 🤔
  /**
    Actions lookup
    @private
    @property _actions
  ...
  */
  _locals(options) {
    let packageName = this.pkg?.name || basename(process.cwd());
  ...
  */
  removePackagesFromProject(packages) {
    ...
    let projectDependencies = { ...this.pkg.devDependencies, ...this.pkg.dependencies };
  ...
  */
  addAddonsToProject(options) {
    ...
    let result = this.taskFor('npm-install').run(taskOptions);
    if (previousCwd) {
  ...
  */
  static lookup(name, options) {
    ...
    let blueprintMap = generateBlueprintMap(options.paths);

    if (blueprintMap.has(name)) {
      return Blueprint.load(blueprintMap.get(name).path, options.blueprintOptions);
    }
  ...
  */
  static load(blueprintPath, blueprintOptions) {
    if (fs.lstatSync(blueprintPath).isDirectory() || fs.lstatSync(blueprintPath).isSymbolicLink()) {
      ...
      if (existsSync(constructorPath)) {
        ...
        if (typeof blueprintModule === 'function') {
          ...
          throw new Error(
            `A core-object based blueprint doesn't work with layer-gen. You need to update it to be class-based. Blueprint ${blueprintPath}`
          );
        }

        // TODO - consider what a non-core-object blueprint would look like 🤔
      }
  ...
  */
  static list() {
    const blueprintMap = generateBlueprintMap();

    return blueprintMap;
  }

  static _existsSync(path, parent) {
    return fs.existsSync(path, parent);
  }

  static _readdirSync(path) {
  ...
  */
  static ignoredFiles = initialIgnoredFiles;

  /**
    @static
    @property ignoredUpdateFiles
  */
  static ignoredUpdateFiles = ['.gitkeep', 'app.css', 'LICENSE.md'];
}
 ...
 *
 * @param {string} path
 * @param {Map<string, object>} blueprintMap
 * @param {object} options
 * @returns
 */
function loadBlueprints(path, blueprintMap, { throwOnClash = true } = {}) {
  let source;

  let packagePath = join(path, 'package.json');
  if (existsSync(packagePath)) {
    source = require(packagePath).name;
  } else {
    throw new Error('blueprints always need a package.json as a sibling');
  }

  const fullPath = join(path, 'blueprints');

  if (existsSync(fullPath)) {
    readdirSync(fullPath).map((fileName) => {
      const blueprintPath = join(fullPath, fileName);

      if (fs.lstatSync(blueprintPath).isDirectory()) {
        //TODO make it so that a layer-gen package.json override (or a local blueprint) can fix this stalemate
        if (throwOnClash && blueprintMap.has(fileName)) {
          // TODO remove this when we implement fully explicit blueprint discovery and see if the test suite passes
          // We removed it because `ember destroy with typescript blueprints` found blueprints in ember-cli and they
          // were already loaded because of the blueprints folder in the root of this repo
          // throw new Error(`Blueprint ${fileName} has already been loaded`);
          return;
        }

        // todo figure out what to do in this map
        blueprintMap.set(fileName, { path: blueprintPath, overridden: blueprintMap.has(fileName), source });
      }
    });
  }
}

function loadBlueprint(path, blueprintMap, { throwOnClash = true, throwOnMissing = false } = {}) {
  if (fs.lstatSync(path).isDirectory()) {
    const blueprintName = basename(path);

    //TODO make it so that a layer-gen package.json override (or a local blueprint) can fix this stalemate
    if (throwOnClash && blueprintMap.has(blueprintName)) {
      throw new Error(`Blueprint ${blueprintName} has already been loaded`);
    }

    // todo figure out what to do in this map
    blueprintMap.set(blueprintName, { path, overridden: blueprintMap.has(blueprintName) });
  } else if (throwOnMissing) {
    throw new Error(`Tired to load blueprint at ${path} but it's missing`);
  }
}
...
function generateBlueprintMap() {
  ...
  allDependencies.forEach((dep) => {
    ...
    try {
      depJsonFile = require.resolve(`${dep}/package.json`, { paths: [process.cwd()] });
    } catch {
      try {
        depJsonFile = findUp.sync('package.json', { cwd: dirname(require.resolve(dep)) });
      } catch {
        // console.log(`Could not load dep ${dep} to check for blueprints`);
        return;
      }
    }
  ...
  // add blueprints folder next to package.json if it exists
  if (packageJsonPath) {
    loadBlueprints(path.dirname(packageJsonPath), blueprintMap, { throwOnClash: false });
  }
